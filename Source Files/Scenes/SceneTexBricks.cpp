...
SceneTexBricks::SceneTexBricks(const shared_ptr<fvec3> clearColor)
{
	...
	auto planeTexture = TextureFactory::fromFile("./assets/models/bricks/bricks.png", true);

	this->gameObjects = {
		shared_ptr<IVisibleGameObject>(new Skybox()),
		shared_ptr<IVisibleGameObject>(new CustomModel("bricks", planeTransform, ShaderFactory::blinnPhong, planeTexture)),
	};
	this->gui = unique_ptr<Gui>(new Gui(clearColor));
}

void SceneTexBricks::updateGameObjects(float deltaTime)
{
...
void SceneTexBricks::renderScene(float currentTime)
{
	for (auto &&object : this->gameObjects)
	{
