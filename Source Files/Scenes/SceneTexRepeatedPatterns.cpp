...
SceneTexRepeatedPatterns::SceneTexRepeatedPatterns(const shared_ptr<fvec3> clearColor)
{
	...
	auto planeTexture = TextureFactory::fromFile("./assets/models/pattern/pattern.png", true);

	this->gameObjects = {
		shared_ptr<IVisibleGameObject>(new Skybox()),
		shared_ptr<IVisibleGameObject>(new CustomModel("pattern", planeTransform, ShaderFactory::blinnPhong, planeTexture)),
	};
	this->gui = unique_ptr<Gui>(new Gui(clearColor));
}

void SceneTexRepeatedPatterns::updateGameObjects(float deltaTime)
{
...
void SceneTexRepeatedPatterns::renderScene(float currentTime)
{
	for (auto &&object : this->gameObjects)
	{
